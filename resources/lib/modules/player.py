...
class player(xbmc.Player):
    ...
    def run(self, title, year, season, episode, imdb, tvdb, url, meta):
        try:
            ...
            if control.setting('preload.mode') == 'true':
                self.done = False
            else:
                self.done = True
        ...
        if overlay == '7':
            ...
            while self.isPlayingVideo():
                try:
                    ...
                    self.currentTime = self.getTime()
                    if self.done == False:
                        if (self.currentTime + 60*5 )>= self.totalTime:
                            self.done = True
                            self.preload()
    ...
    def preload(self):
        try:
            episode = str(int(self.episode) + 1)
            season = self.season
            ...
            control.execute('RunPlugin(plugin://%s)' % ('plugin.video.lastship/?action=preload&imdb=' + imdb + '&tvshowtitle=' + tvshowtitle + '&year=' + year + '&tvdb=' + tvdb + '&episode=' + episode + '&season=' + season))
        except: pass
